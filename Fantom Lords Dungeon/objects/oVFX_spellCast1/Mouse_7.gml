...
{
	if (oLord1.spellCast = 0) //Deadhour
	{
		if (enemy != noone && !enemy.obstacle)
		{
			...
			else if enemy.x = oLord1.x+64 && enemy.y = oLord1.y-64 enemy.target = true;
			else if enemy.x = oLord1.x+64 && enemy.y = oLord1.y+64 enemy.target = true;
			else if enemy.x = oLord1.x-64 && enemy.y = oLord1.y+64 enemy.target = true;
			else if enemy.x = oLord1.x-64 && enemy.y = oLord1.y-64 enemy.target = true;
			...
			if enemy.target = true
			{
				...
				{
					global.dmgLORD = (oLord1.thisLord_MAG*2) + irandom_range(-6,12);
					if global.dmgLORD <= 0 global.dmgLORD = 1;		
				}
				else if (current_hour > 6 && current_hour < 12)
				{
					global.dmgLORD = round(oLord1.thisLord_MAG/2) + irandom_range(-3,6);
					if global.dmgLORD <= 0 global.dmgLORD = 1;		
				}
				else
				{
	...
	else if (oLord1.spellCast = 1) //Matchup
	{
		if (enemy != noone && enemy != obstacle)
		{
	...
	else if (oLord1.spellCast = 5) //Blizzard
	{
	...
	else if (oLord1.spellCast = 7) //Lazerain
	{
		if (collision_point(x,y,oLord1,false,true))
		{
			var struct =
			{
				...
				beamDamage : oLord1.thisLord_MAG
			}

			oLord1.spellCast = noone;
			oLord1.thisLord_ACTpoints -= 2;
			instance_create_layer(oLord1.x+oLord1.sprite_width/2,oLord1.y+oLord1.sprite_height/2,"VFX",oSKILL_WEAPON_Lazerain, struct);
			instance_destroy();
		}
	}
	...
	else if (oLord1.spellCast = 10) //Berserk
	{
		if (collision_point(x,y,oLord0Parent,0,false))
		{
			oLord1.spellCast = noone;
	...
	else if (oLord1.spellCast = 13) //Tower
	{
		if (collision_point(x,y,oLord1,false,true))
		{
			...
			instance_create_depth(oLord1.x+32,oLord1.y+32,depth+1,oSKILL_WEAPON_Tower_back);
			instance_destroy();
		}
	}	
}
