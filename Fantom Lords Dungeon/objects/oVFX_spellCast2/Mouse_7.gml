...
if oLord2.thisLord_ascend
{
	...
	else if (oLord2.spellCast = 1) //Matchup
	{
		...
		if (targetEnemy != noone && targetEnemy != obstacle)
		{
			...
			if (atk >= def && atk >= mag)
			{
				var struct1 = { choice : "def" , buff : oLord2.thisLord_MAG };
				var struct2 = { choice : "atk" , debuff : oLord2.thisLord_MAG };
				
				instance_create_layer(oLord2.x+32,oLord2.y+32,"VFX",oSKILL_WEAPON_Matchup1,struct1);
			...
			{
				var struct1 = { choice : "atk" , buff : oLord2.thisLord_MAG };
				var struct2 = { choice : "def" , debuff : oLord2.thisLord_MAG };
			...
			{
				var struct1 = { choice : "mag" ,  buff : oLord2.thisLord_MAG };
				var struct2 = { choice : "mag" ,  debuff : oLord2.thisLord_MAG };
	...
	else if (oLord2.spellCast = 2) //Spirit Arrows
	{
	...
	else if (oLord2.spellCast = 3) //Eerie Slash
	{
		if (enemy != noone && enemy.target)
		{
			global.dmgLORD = ((oLord2.thisLord_MAG + oLord2.thisLord_ATK) + irandom_range(-3,6));
			if global.dmgLORD <= 0 global.dmgLORD = 1;				   
			oLord2.spellCast = noone;
			oLord2.thisLord_ACTpoints -= 2;
	...
	else if (oLord2.spellCast = 4) //Ghosts
	{
		if (collision_point(x,y,oLord2,false,true))
		{
			var struct = { dmg : oLord2.thisLord_MAG };
			
			oLord2.spellCast = noone;
			oLord2.thisLord_ACTpoints -= 2;
			instance_create_layer(oLord2.x+32,oLord2.y+32,"VFX",oSKILL_WEAPON_GhostMaker, struct);
			instance_destroy();
		}
	}
	...
	else if oLord2.spellCast = 8 //Bloodshed
	{
			if collision_point(x,y,oLord2,false,true)
			{
				...
				if global.dmgLORD <= 0 global.dmgLORD = 1;
				instance_create_layer(oLord2.x+32,oLord2.y+32,"VFX",oSKILL_WEAPON_Bloodshed,struct);
				instance_destroy();
			}
	}
	else if (oLord2.spellCast = 9) //Double Strike
	{
		if (enemy != noone && enemy.target)
		{
			global.dmgLORD = (oLord2.thisLord_ATK + oLord2.thisLord_ACT + irandom_range(-4,4));
			if global.dmgLORD <= 0 global.dmgLORD = 1;				   
			oLord2.spellCast = noone;
			oLord2.thisLord_ACTpoints -= 2;
	...
	else if (oLord2.spellCast = 11) //Agility
	{
		if (collision_point(x,y,oLord2,false,true))
		{
			global.castLord = oLord2;
			oLord2.spellCast = noone;
			oLord2.thisLord_ACTpoints -= 2;
			instance_create_layer(oLord2.x+32,oLord2.y+32,"VFX",oSKILL_WEAPON_Agility);
			instance_destroy();
		}
	}	
	else if oLord2.spellCast = 12 //Incense
	{
			if collision_point(x,y,oLord2,false,true)
			{
				global.castLord = oLord2;
