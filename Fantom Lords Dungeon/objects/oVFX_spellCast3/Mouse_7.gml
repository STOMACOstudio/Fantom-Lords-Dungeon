...
{
	if (oLord3.spellCast = 0) //Deadhour
	{
		if (enemy != noone && !enemy.obstacle)
		{
			...
			if enemy.target = true
			{
				...
				{
					global.dmgLORD = (oLord3.thisLord_MAG*2) + irandom_range(-6,12);
	...
	else if (oLord3.spellCast = 1) //Matchup
	{
	...
	else if (oLord3.spellCast = 3) //Eerie Slash
	{
		if (enemy != noone && enemy.target)
		{
			global.dmgLORD = ((oLord3.thisLord_MAG + oLord3.thisLord_ATK) + irandom_range(-3,6));
			if global.dmgLORD <= 0 global.dmgLORD = 1;				   
			oLord3.spellCast = noone;
			oLord3.thisLord_ACTpoints -= 2;
	...
	else if (oLord3.spellCast = 4) //Ghosts
	{
		if (collision_point(x,y,oLord3,false,true))
		{
			var struct = { dmg : oLord3.thisLord_MAG };
			
			oLord3.spellCast = noone;
			oLord3.thisLord_ACTpoints -= 2;
			instance_create_layer(oLord3.x+32,oLord3.y+32,"VFX",oSKILL_WEAPON_GhostMaker, struct);
			instance_destroy();
		}
	}
	else if (oLord3.spellCast = 5) //Blizzard
	{
	...
	else if (oLord3.spellCast = 7) //Lazerain
	{
		if (collision_point(x,y,oLord3,false,true))
		{
			var struct =
			{
				...
				beamDamage : oLord3.thisLord_MAG
			}

			oLord3.spellCast = noone;
			oLord3.thisLord_ACTpoints -= 2;
			instance_create_layer(oLord3.x+oLord3.sprite_width/2,oLord3.y+oLord3.sprite_height/2,"VFX",oSKILL_WEAPON_Lazerain, struct);
			instance_destroy();
		}
	}
	else if oLord3.spellCast = 8 //Bloodshed
	{
			if collision_point(x,y,oLord3,false,true)
			{
				...
				if global.dmgLORD <= 0 global.dmgLORD = 1;
				instance_create_layer(oLord3.x+32,oLord3.y+32,"VFX",oSKILL_WEAPON_Bloodshed,struct);
				instance_destroy();
			}
	}
	...
	else if (oLord3.spellCast = 10) //Berserk
	{
		if (collision_point(x,y,oLord0Parent,0,false))
		{
			oLord3.spellCast = noone;
	...
	else if (oLord3.spellCast = 13) //Tower
	{
		if (collision_point(x,y,oLord3,false,true))
		{
			...
			instance_create_depth(oLord3.x+32,oLord3.y+32,depth+1,oSKILL_WEAPON_Tower_back);
			instance_destroy();
		}
	}	
}
